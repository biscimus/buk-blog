...
> ...I'm sorry, what did an undecidable language mean again?
...
Right. Genereally speaking, if there is a program $$M$$ able to give a yes-or-no answer whether the word is in a certain language $$L$$ or not for every word over the alphabet, we say the program $$M$$ **decides** $$L$$ or $$M$$ is a **decider** of $$L$$. Furthermore, we say that the language $$L$$ is **decidable**. In the computer science context, we typically work with the binary alphabet $$\{0, 1\}$$.

The algorithm above actually works for all languages as long as the language is finite. We can simply iterate over our dictionary and check for its membership given an input. It can therefore be said that ___all finite languages are decidable.___
...
###### **$$H$$:** _Given a program and an input, does the program terminate on the input?_
...
Let's try our method on another well-known indecidable problem known as the **$$\bm{\varepsilon}$$-Halting Problem**.

###### **$$H_{\varepsilon}$$:** Given a program, does the program terminate on an empty input?
...
function isWordInHaltingProblem(func, input: string): boolean {
    // Uses inWordInEpsilonHaltingProblem somehow
}
```
...
###### **$$H$$:** _Given a Turing machine $$M$$ and an input $$w$$, does $$M$$ halt on $$w$$?_
...
###### **$$H$$:** _Given an encoding of a Turing machine $$\langle M \rangle$$ and an input $$w$$, does $$M$$ halt on $$w$$?_

### 4. Putting It All Together

> I think I got a slight grasp of what you're trying to say. But how does this help me pass my computability test?

We just need to translate our previous examples into the notation used in the lectures. Let's look at the $$\varepsilon$$-halting problem again:

###### **$$H_{\varepsilon}$$:** Given a Turing machine $$M$$'s encoding $$\langle M \rangle$$, does $$M$$ halt on empty input?

The lectures use a set notation to define languages, so what you would usually see is something like this:

###### $$H_{\varepsilon} = \{ \langle M \rangle \mid M \text{ halts on } \varepsilon \}$$.

The steps we took to prove the indecidibility of the $$\varepsilon$$-halting problem can be summarized as follows:

Introduction

Assume $$H_{\varepsilon}$$ is decidable. Then, there must be a Turing machine $$M_{\varepsilon}$$ deciding $$H_{\varepsilon}$$.
...
Using $$M_{\varepsilon}$$, we construct a Turing machine $$M_{H}$$ deciding the halting problem $$H$$.

Given an encoded input $$w'$$, we check whether $$w'$$ matches the syntax of $$\langle M \rangle w$$ for some Turing machine $$M$$ and input $$w$$. If not, we reject the input. This step is needed since we're dealing with mere binary strings here.

Given a valid input $$\langle M \rangle w$$, we construct the Gödel number of a new Turing machine $$M_1$$, which acts as follows:

- On empty input, $$M_1$$ simulates $$M$$ on input $$w$$.
- On any other input, $$M_1$$ goes into an infinite loop.

We then feed $$\langle M_1 \rangle$$ into $$M_{\varepsilon}$$. If $$M_{\varepsilon}$$ accepts, we accept; otherwise, we reject.
...
We prove the correctness of our newly built $$M_{H}$$. Invalid encodings are rejected right away. For valid encodings $$\langle M \rangle w$$, we must show that $$M_H$$ halts on every input and $$M \text{ halts on } w \Leftrightarrow M_H \text{ accepts } \langle M \rangle w$$.

$$(\Rightarrow)$$: Let $$M$$ halt on $$w$$. Then, $$M_1$$ halts on empty input, meaning $$\langle M_1 \rangle$$ is a positive instance of $$H_{\varepsilon}$$. As a result, $$M_{\varepsilon}$$ accepts $$\langle M_1 \rangle$$, meaning $$M_H$$ accepts $$\langle M \rangle w$$.

$$(\Leftarrow)$$: Let $$M$$ not halt on $$w$$. Then, $$M_1$$ does not halt on empty input, meaning $$\langle M_1 \rangle$$ is a negative instance of $$H_{\varepsilon}$$. As a result, $$M_{\varepsilon}$$ rejects $$\langle M_1 \rangle$$, meaning $$M_H$$ rejects $$\langle M \rangle w$$.

This shows that $$M_H$$ is a decider for the halting problem $$H$$. This contradicts the fact that $$H$$ is indecidable. Thus, our initial assumption that $$H_{\varepsilon}$$ is decidable must be wrong. $$H_{\varepsilon}$$ is indecidable. $$\square$$

### 5. Further Examples

All examples below leverage the halting problem to prove its undecidibility. Try to understand how each example works and how to construct similar proofs on your own.

###### **$$H_{never}$$:** _Given a program as input, does the program halt on no input at all?_
...
###### **$$H_{all}$$:** _Given a program as input, does the program halt on every input?_
...
###### **$$A_{all}$$:** _Given a program as input, does the program return `true` on every input?_
...
###### **$$H_{42}$$:** _Given a program as input, does the program halt on the input 42?_
...
###### **$$H_{special}$$:** _Given a program `func` as input, does `func(func)` return `true`?_
...
###### **$$A_{\lvert 7 \rvert}$$:** _Given a program as input, does the program return `true` for exactly 7 different inputs?_
