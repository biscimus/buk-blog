...
Do you see why? We're checking whether an input program `func` returns 1 on input 1, therefore effectively checking whether the partial function computed by `func` is in $$\mathcal{S} = \{ f \mid f(1) = 1 \}$$.
