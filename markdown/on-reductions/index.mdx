...
We now focus on a special variant called an _Entscheidungsproblem_ (Entscheidung: decision). In an entscheidungsproblem, the problem instances' answers can only be true or false. In other words, they're all yes-and-no questions!
...
> Let $$A, B$$ be two languages over an alphabet $$\Sigma$$. $$A$$ is reducible to $$B$$  $$(A \leq B)$$, if there exists a computable function $$f: \Sigma^* \rightarrow \Sigma^*$$ so that $$x \in A \Leftrightarrow f(x) \in B$$ for all $$x \in \Sigma^*$$.

One way to visualize this is to imagine our $$\Sigma^*$$ is some kind of discrete metric space with languages $$A, B$$ corresponding to shapes defined in $$\Sigma$$. The reduction $$f$$ as a [transformation](https://en.wikipedia.org/wiki/Transformation_(function)), warping the entire space so that $$A$$ fits inside $$B$$ and $$\overline{A}$$ inside $$\overline{B}$$. The following animation might help.
...
Also, being able to reduce A to B doesn't mean B is necessarily harder than A, since you can reduce a problem to an identical problem and still call it a reduction; If faced with a second burning bin, the computer scientist can calmly walk away, knowing that this can be reduced to the previous burning bin problem. This, however, doesn't mean that the previous burning bin was a harder problem to solve.
