...
Reducing A to B has a couple implications for the _relative difficulty_ of A and B. Intuitively, it is clear that if we can solve B, we can surely solve A as well. Furthermore, if we know that if we can't solve A, we can say that B is impossible to solve as well. Likewise, if B is easy to solve, A is easy to solve. If A is really difficult, we surely know B won't be easier than A. Note that being able to reduce A to B doesn't mean B is necessarily harder than A since you can reduce a problem to an idential problem and still call it a reduction (burning bin reduced to a burning bin is still a burning bin).
