...
> Well, that depends. Computers with better processors would take less time to run the same code. Plus, JS is much slower compared to C. I'm pretty sure I could do that same thing in C but faster.

Exactly. As you stated, the execution of an algorithm measured in time depends on multiple factors. We therefore need a unit for the runtime which is ***independent from time*** in the physical sense.  
...
Unlike the first example, the runtime of this function depends on the input number. If we assume the `console.log("Hello World!")` part takes a constant runtime of $$c$$, then the runtime of the function would be around $$c \cdot n$$. In this case, we say the function above has a *linear* runtime.

> Okay, but I still don't get why we need the O-notation. 

Well, the O-notation helps us to formally group functions with similar runtime behavior into the same box, known as *complexity classes*. Suppose we want to group all programs with linear runtime in the same group. How would you describe the runtime of the function below?
...
Exactly. Both $$2cn + 2c$$ and $$cn$$ are linear functions, identical apart from the principal coefficient and a constant term. The O-notation helps us to ignore these factors and focus on the most significant part of the runtime.
