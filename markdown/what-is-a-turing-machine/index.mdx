...
Around a century ago, before world wars needed to be numbered and the field of computer science was conceived, a big question in the realm of mathematics emerged known as the [__Entscheidungsproblem__](https://en.wikipedia.org/wiki/Entscheidungsproblem). Posed by David Hilbert in 1928, the problem asked:

__Can every mathematically formulated problem be answered using a system of formally defined steps?__

In other words: __Is there a problem impossible to solve with any algorithm?__

During his attempt to solve the it, Alan Turing designed a special machine: Not a machine in the physical sense, but a mathmatical automaton, named the __Turing Machine__ in the present day. It is defined as follows:

<div className="flex flex-col sm:flex-row justify-center items-center border dark:border-orange-300/40 rounded-md p-12 sm:p-4">
    <div className="flex flex-col gap-4 items-start sm:w-[50%] sm:items-end">
        <div className="text-xl">$$M = (Q, \Sigma, \Gamma, B, q_0, \overline{q}, \delta),$$</div>
        <div>where</div>
    </div>
