...
export default function TuringMachine() {
    ...
    return (
        <div className={`${mono.className} flex flex-col md:flex-row gap-4 justify-stretch`} >
                            ...
                            <div
                                className={`w-12 h-12 p-2 text-center font-medium transition-all duration-200 ${
                                    index === head 
                                        ...
                                        : 'dark:bg-slate-600 border-2 border-gray-400 dark:border-slate-500 text-gray-800 dark:text-slate-200 hover:bg-gray-400 dark:hover:bg-slate-500'
                                }`}
                            >
                                {element !== "B" ? element : ""}
                            </div>
            ...
            <div className="flex flex-col justify-center items-center p-6 dark:bg-slate-800 rounded-lg border border-gray-300 dark:border-slate-600 gap-4 overflow-auto">
...
export function TransitionTable({
    ...
    return (
        <div className="dark:bg-slate-800 rounded-lg p-4 border border-gray-300 dark:border-slate-600">
            {/* Table header */}
            <div className="grid grid-cols-[60px_1fr_1fr_1fr] gap-2 mb-4 text-sm font-medium">
                ...
                <div className="dark:bg-slate-700 text-gray-800 dark:text-slate-200 text-center py-2 px-3 rounded border border-gray-300 dark:border-slate-600">0</div>
                <div className="dark:bg-slate-700 text-gray-800 dark:text-slate-200 text-center py-2 px-3 rounded border border-gray-300 dark:border-slate-600">1</div>
                <div className="dark:bg-slate-700 text-gray-800 dark:text-slate-200 text-center py-2 px-3 rounded border border-gray-300 dark:border-slate-600">B</div>
            </div>
            ...
            <div className="grid grid-cols-[60px_1fr_1fr_1fr] gap-2 max-h-[348px] overflow-auto">
                {Object.entries(transitions).map(([state, transition]) => (
                    <React.Fragment key={state}>
                        {/* State Button */}
                        <button
                            className={`relative group dark:bg-slate-700 dark:hover:bg-slate-600 border border-gray-300 dark:border-slate-600 rounded py-2 px-3 text-sm transition-colors duration-200 ${Number(state) === 1 ? 'cursor-not-allowed' : ''}`}
                            ...
                            return (
                                <div
                                    ...
                                    className={`rounded border p-1 space-y-1 transition-all duration-200 ${isHighlighted
                                            ? 'bg-green-700/60 border-green-500 shadow-lg shadow-green-500/20'
                                            : 'dark:bg-slate-700 border-gray-300 dark:border-slate-600'
                                        }`}
                                >
                                    {/* State Select */}
                                    <select
                                        className="w-full bg-transparent dark:bg-slate-600 border border-gray-400 dark:border-slate-500 rounded text-center text-gray-800 dark:text-slate-200 text-xs py-1 hover:bg-gray-400 dark:hover:bg-slate-500 focus:ring-1 focus:ring-orange-400 dark:focus:ring-slate-400 focus:border-orange-400 dark:focus:border-slate-400 transition-colors duration-200"
                                    ...
                                    >
                                        {states.map(stateOption => <option key={stateOption} value={stateOption} className="dark:bg-slate-600 rounded-none">q{stateOption}</option>)}
                                    </select>
                                    ...
                                    >
                                        <option value="0" className="dark:bg-slate-600 rounded-none">0</option>
                                        <option value="1" className="dark:bg-slate-600 rounded-none">1</option>
                                        <option value="B" className="dark:bg-slate-600 rounded-none">B</option>
                                    </select>
                                    ...
                                    >
                                        <option value="L" className="dark:bg-slate-600 rounded-none">L</option>
                                        <option value="N" className="dark:bg-slate-600 rounded-none">N</option>
                                        <option value="R" className="dark:bg-slate-600 rounded-none">R</option>
                                    </select>
                                </div>
                            );
                        })}
            ...
            <div className="mt-4 flex justify-center">
                <button
                    onClick={onAddState}
                    className="dark:bg-slate-700 hover:bg-gray-300 dark:hover:bg-slate-600 text-gray-800 dark:text-slate-200 border border-gray-300 dark:border-slate-600 py-2 px-4 rounded transition-colors duration-200 flex items-center gap-2"
                >
